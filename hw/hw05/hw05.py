...
def repeated(t, k):
    ...
    assert k > 1
    "*** YOUR CODE HERE ***"
    def findKTimes(prex, times):
        if times == k:
            return prex
        curr = next(t)    
        if prex == curr:
            return findKTimes(curr, times + 1)
        else:
            return findKTimes(curr, 1)
    return findKTimes(next(t), 1)        
...
def merge(incr_a, incr_b):
    ...
    while next_a is not None or next_b is not None:
        #print("next_a is ", next_a," next_b is", next_b)
        if next_a is None:
            yield next_b
            ...
            next_b = next(iter_b, None)
    """
    def doMerge(next_a, next_b):
        if next_a is None and next_b is None:
            return
        elif next_a is None:
            yield next_b
            yield from doMerge(next_a, next(iter_b, None))
        elif next_b is None:
            yield next_a
            yield from doMerge(next(iter_a, None), next_b)
        elif next_a < next_b:
            yield next_a
            yield from doMerge(next(iter_a, None), next_b)
        elif next_a > next_b:
            ...
            yield next_a
            yield from doMerge(next(iter_a, None), next(iter_b, None))          
    return doMerge(next_a, next_b)
    """
