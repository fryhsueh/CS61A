...
def subseqs(s):
    ...
    if not s:
        ...
        newList = subseqs(s[1:])
        return insert_into_all(s[0], newList) + newList
...
def inc_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            ...
            return [[s[0]]] +  subseq_helper(s[1:], prev)
        else:
            a = inc_subseqs(s)
            #b = 
            return insert_into_all(prev, a) + a
    return [[]] if s == [] else subseq_helper(s[1:], s[0])
...
def trade(first, second):
    ...
    while not equal_prefix() and m <= len(first) and n <= len(second):
        #print("sum of first:", sum(first[:m]), " sum of second:", sum(second[:n]))
...
def make_glookup(class_assignments):
    ...
    "*** YOUR CODE HERE ***"
    curr, total = 0, 0
    def glookup(course, score):
        nonlocal curr, total
        curr += score
        total += class_assignments[course]
        return curr / total
    return glookup
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    thinking:
        for four-leaves trees:
            * * * * => [* *] * * or * [* *] * or * * [* *] like three num_trees(3)
                    or [* *][* *] like a num_trees(2)
    """
    if n <= 2:
        return 1
    else:
