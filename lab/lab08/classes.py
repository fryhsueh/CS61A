...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        def take_over(card, target):
            copy = target.deck.cards[:]
            for c in copy:
                c.attack = c.attack + card.attack
                c.defense = c.defense + card.defense
            target.deck.cards = copy
