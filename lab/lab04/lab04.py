...
def distance(city_a, city_b):
    ...
    return computeDistance(get_lon(city_a), get_lat(city_a), get_lon(city_b), get_lat(city_b))    
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    city = make_city('new city', lat, lon)
...
def add_chars(w1, w2):
    ...
    def rec(w1, w2, out):
        if len(w1) == 0:
            ...
            if w1[0] == w2[0]:
                return rec(w1[1:], w2[1:], out)
            else:
                return rec(w1, w2[1:], out + w2[0])
